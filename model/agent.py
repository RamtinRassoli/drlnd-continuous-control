...
class Agent:
    ...
    def __init__(self, state_size, action_size, random_seed, config):
        ...
        self.actor_optimizer = optim.Adam(self.actor_local.parameters(), lr=self.config['ddpg']['lr_acotor'])

        # Critic Network (w/ Target Network)
        self.critic_local = Critic(state_size, action_size, random_seed).to(device)
        self.critic_target = Critic(state_size, action_size, random_seed).to(device)
        self.critic_optimizer = optim.Adam(self.critic_local.parameters(),
                                           lr=self.config['ddpg']['lr_critic'],
                                           weight_decay=self.config['ddpg']['weight_decay'])

        # Noise process
        self.noise = OUNoise(action_size, random_seed)

        # Replay memory
        self.memory = ReplayBuffer(action_size, self.config['exp_replay']['buffer'],
                                   self.config['exp_replay']['batch'], random_seed)
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        if len(self.memory) > self.config['exp_replay']['batch']:
...
class OUNoise:
    ...
    def sample(self):
        ...
        # dx = self.theta * (self.mu - x) + self.sigma * nprandom.standard_normal(1)
